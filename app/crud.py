...
def save_image_in_db(db, title, data, filename, url="", description=""):
    #db_item = FileContent(**item.dict(), owner_id=user_id)
    render_file = render_picture(data)
    db_item = FileContent(title=title, data=data, filename=filename, rendered_data=render_file, url=url, description=description)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
